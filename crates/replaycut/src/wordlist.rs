//! The passphrase generator behind `GET /api/password/suggest` (since 2.8).
//!
//! The word list is the EFF Short Wordlist #1 (1296 short, distinct words,
//! <https://www.eff.org/dice>), by the Electronic Frontier Foundation,
//! licensed CC-BY 3.0 US <https://creativecommons.org/licenses/by/3.0/us/>.
//! Four words drawn from it carry about 41 bits, which is what a password
//! for a service on the local network needs to survive a throttled login.

use argon2::password_hash::rand_core::{OsRng, RngCore};

/// The EFF Short Wordlist #1, in the order the list has them.
pub static WORDS: [&str; 1296] = [
...
/// One word, drawn uniformly (rejection sampling, no modulo bias). The
/// list's single hyphenated entry is skipped so that the dashes of a
/// passphrase separate exactly as many words as were drawn.
fn word() -> &'static str {
    loop {
        let mut bytes = [0u8; 2];
        OsRng.fill_bytes(&mut bytes);
        let n = u16::from_le_bytes(bytes) as usize;
        if n >= 64_800 {
            // 64800 = 50 * 1296: everything above would favour the first words
            continue;
        }
        let candidate = WORDS[n % WORDS.len()];
...
/// `n` words joined with `-`, for example `orbit-velvet-cactus-lantern`.
pub fn passphrase(n: usize) -> String {
...
mod tests {
    ...
    #[test]
    fn the_list_is_the_eff_short_wordlist() {
        assert_eq!(WORDS.len(), 1296);
        let mut sorted = WORDS.to_vec();
        sorted.sort_unstable();
        sorted.dedup();
        assert_eq!(sorted.len(), 1296, "the list has a duplicate");
    ...
    #[test]
    fn a_passphrase_is_four_words_and_differs_every_time() {
        let a = passphrase(4);
        let parts: Vec<&str> = a.split('-').collect();
        assert_eq!(parts.len(), 4, "{a}");
        assert!(parts.iter().all(|p| WORDS.contains(p)), "{a}");
        let b = passphrase(4);
        assert_ne!(a, b, "two passphrases in a row are the same");
    }

    #[test]
    fn every_word_can_be_drawn() {
        // the draw must reach both ends of the list, not just the middle
        let mut first = false;
        let mut last = false;
        for _ in 0..200_000 {
            let w = word();
            first |= w == WORDS[0];
            last |= w == WORDS[WORDS.len() - 1];
            if first && last {
                return;
            }
        }
        panic!("the draw does not reach the whole list");
    }
}
