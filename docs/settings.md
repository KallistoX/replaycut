...
| `cleanup` | Since 3.0: `{ "afterShare": "done", "recycleDoneAfterDays": 0 }` - what happens to a clip once it has been shared. `afterShare` is where the "Afterwards" pill under the player starts (Settings › Quick share): `keep` leaves the clip in the list, `done` (the default) takes it out (one click brings it back), `recycle` also moves the recording to the recycle bin. The share may say otherwise per job (`after` in `POST /api/share`). `recycleDoneAfterDays` moves the recordings of clips that have been done that long to the recycle bin; `0` never does. **Cuts and shared files are never removed automatically** - the cut is what every later rendering is made from. |
| `quickShare` | Since 3.13: what a quick share (Enter on the clips page) takes when nothing else is chosen - `audio` (`mix`, `gamemic`, `game`, `gamediscord`), `mode` (`h264` or `copy` = as recorded), `frame` (`wide` or `vertical`; `vertical` needs `h264`) and `subtitles` (`off`, or `burn`: read the speech first and burn it in unread - with the beta switched on only). Afterwards and the target have their fields already: `cleanup.afterShare` and `integrations.<id>.quickShare`. The pills under the player start here and change the next share only. |
| `subtitles` | Since 3.11, **beta and off**: `{ "enabled": false, "model": "small", "language": "auto", "source": "auto", "gpu": false, "look": { … } }`. With `enabled` false nothing here runs: no cut shows a subtitle block, Settings › Quick share offers no subtitles, no model is downloaded and no request goes to Hugging Face. `model` is `base`, `small` (the default - it reads game jargon far better for about nine percent more time) or `medium`, or the name of a file you put in the models folder yourself. `language` is `auto` or an ISO 639-1 code. `auto` works, but whisper decides again for every window it looks at: on a German recording two quiet passages came back as "And" and "Yeah" where a fixed `de` read "Und" and "Ja". It also never reports which language it settled on - neither the SRT nor the JSON it writes carries one - so a transcript read with `auto` leaves its subtitle track untagged. Naming your language is worth the one setting. `source` is `auto`, `mic` or `mix`. `auto` reads the mix - every track, in-game voice included (since 3.14; 3.11 to 3.13 read the microphone alone when there was one, and the others' lines were missing). `mic` is the microphone alone: the track named for it, else the one OBS says carries only a microphone, else track 2 of a four-track recording. Microphone and voice chat together, without the game, is chosen for a single cut in the workshop and is not a setting. When two people talk at the same time, whisper usually writes down only one of them. `gpu` is off: the card belongs to the game, and in the ffmpeg builds this was measured on the filter runs on the CPU whatever this says. `look` (since 3.12) is how burned-in subtitles look unless a cut has a look of its own: for `wide` (16:9) and `vertical` (9:16) each a `position` (`lower`, `middle`, `top`), a `size` (`s`, `m`, `l`) and a `color` (`white`, `box`, `yellow`) - named steps, so the preview in the workshop and the rendering agree; the default is lower · M · white in both. The numbers of 3.11's `style` are not carried over: a file that still has them reads, and they go with the next save. The text is drawn in a font that comes with the build (Inter Bold, SIL Open Font License 1.1), so a rendering looks the same on every machine. |
...
| `integrations.youtube` | YouTube (since 2.6): every share is uploaded as its own video. `privacy` is `unlisted` (default), `private` or `public`; `description` is a template with `{title}`, `{clip}` and `{date}`. The video title is the clip's title (or its name), plus `#Shorts` for a vertical cut. `client` (since 3.3) is `builtin` (default: the Google client this release was built with, nothing to set up) or `own` (a client from your own Google project, credential `replaycut/youtube-client`, see [`docs/youtube.md`](youtube.md)); either way the channel is the credential `replaycut/youtube`. `clientType` is `tv` (a "TVs and Limited Input devices" client, connected with a code from any device, the default) or `desktop` (a "Desktop app" client, connected in the browser on this PC). Changing `client` or `clientType` disconnects the channel, because a refresh token belongs to the client that issued it. |
