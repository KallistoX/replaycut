...
| `mockup-workshop-3.12.html`, `workshop.css` | The clips page of 3.12: a recording opens into its cuts in the list, one player plays either, and a loaded cut gets the tools of the workshop - the caption lane, the line inspector, the look of its subtitles, the render row, its outputs - at 1280 px, 700 px and on a phone, with every menu, the keyboard of both modes and the new components in all their states. Each frame carries `data-frame` (`F1` ... `F19`, then the frames marked "later"), and the ids are today's wherever the element exists today. `workshop.css` holds only the new classes and the changes to existing ones, marked `CHANGED`. Where the page and the sheet differ, the page is right: speech is read from the microphone or the mix and nothing else, a target is a storage or "File on this PC" (a webhook only posts), the render row has a fifth pill for the quality, the default look of a 9:16 rendering is the lower third, and a line carries no language of its own. |
