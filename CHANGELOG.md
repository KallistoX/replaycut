...
no speech said "none yet" and "Subtitles read". Next is the rest of the
workshop - adding lines and typing them yourself, dragging edges, zoom,
the phone.

### Fixed

- **A transcription that hears nothing says so** (beta). A cut whose speech
  whisper could not read - one short word is easily taken for noise -
  showed "This cut has none yet" after the run, and the result said
  "Subtitles read". The job now says how many lines it read (`lines`), and
  the workshop tells the two apart.
...
them. They are beta and switched off; Settings › Subtitles turns them on
and fetches the one thing that has to come from outside, the model.
...
- **No more cuts that stay "cutting …" forever.** A share reserves its cut
  ...
  cut goes with its share now, and a start clears what an earlier stop left.
